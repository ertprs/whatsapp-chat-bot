...
app.post('/sms', (req, res) => {
    ...
    if (userArr.length == 1) {
        ...
        if (userArr[1] == 1) {
            ...
            if (!userArr[2]) {
                ...
                if (!userArr[3]) {
                    ...
                    if (parseInt(userArr[3]) > 0 && parseInt(userArr[3]) < 150) {
                        ...
                        if (!userArr[4]) {
                            //Ask for gender
                            resp.message(`Tell us patient's gender as well\n\n💬 1 for Male 👨‍🦰\n💬 2 for Female 👩‍🦰\n💬 3 for Other 🧑`);
                            req.session.info = userArr;
                            res.status(200).send(resp.toString());
                            ...
                            if (parseInt(userArr[4]) >= 1 && parseInt(userArr[4]) <= 3) {
                                ...
                                resp.message(`That does not seem like a valid option.\n Select 1,2 or 3.\n\nTell us patient's gender as well\n1 for Male 👨‍🦰\n2 for Female 👩‍🦰\n3 for Other 🧑`);
                        ...
                        //Age not in range
                        resp.message(`🙅‍♀️ That does not seem like a valid age.\n\nWhat is the patient's age, in years?Ex. 25`);
                        req.session.info.pop();
                        res.status(200).send(resp.toString());
                    }
            ...
            if (!userArr[2]) {
                ...
                if (!userArr[3]) {
                    ...
                    if (parseInt(userArr[3]) > 0 && parseInt(userArr[3]) < 150) {
                        if (!userArr[4]) {
                            //Ask for gender
                            resp.message(`कृपया हमें पेशंट का लिंग बताएं\n\n💬 पुरुष के लिए 1 👨‍🦰\n💬 महिला के लिए 2\n💬 अन्य के लिए 3 🧑`);
                            req.session.info = userArr;
                            res.status(200).send(resp.toString());
                            ...
                            if (parseInt(userArr[4]) >= 1 && parseInt(userArr[4]) <= 3) {
                                ...
                                resp.message(`यह एक वैध विकल्प की तरह नहीं लगता है। 1,2 या 3 भेजें।\n\nकृपया हमें पेशंट का लिंग बताएं\nपुरुष के लिए 1 👨‍🦰\nमहिला के लिए 2 👩‍🦰\nअन्य के लिए 3 🧑`);
                                req.session.info.pop();
                                res.status(200).send(resp.toString());
                        ...
                        resp.message(`🙅‍♀️ यह एक वैध उम्र की तरह नहीं लगता है।\nपेशंट की उम्र, वर्षों में क्या है?\nउदाहरण- 25`);
                        req.session.info.pop();
                        res.status(200).send(resp.toString());
                    }
            ...
            resp.message(`You did not select a valid number\n\nHello!\nनमस्ते 🙏\nWelcome to Doctor on Call service 👩‍⚕️\nडॉक्टर ऑन कॉल सेवा में आपका स्वागत है👨‍⚕️\n- 💬 Type and send 1 for English\n- 💬 हिंदी के लिए 2 लिख कर भेजे`);
            req.session.info = [];
            res.status(200).send(resp.toString())
